...
%   === Adapted by Xin Liu, Washington University in St. Louis, 2023 ===================
%   === Adapted by Ruihong Zou, Queensland Univerisity of Technology, 2023 ===================
...
if( exist( [ CALCIUM_FLUORESCENCE_file ] , 'file' ) == 2 )
    ...
    if( Q_presets( 'ALL' ) || Q_presets( 'SPIKE-PROBABILITY-RASTER' ) )
        ...
        [ directory , name , ~ ] = fileparts( CALCIUM_FLUORESCENCE_file );
        OUTPUT_PATH = [ directory '/' name '.mat' ];
        ...
        if( exist( OUTPUT_PATH , 'file' ) ~= 2 )
            ...
            save( '20180514-f1-data.mat' , '-v7' , '-struct' , 'output_args' );
            fileattrib( OUTPUT_PATH , '+w' , 'g' );
            ...
            OUTPUT_PATH = [ directory '/' name '_SPIKE.mat' ];
            fileID=fopen(OUTPUT_PATH, 'w');
            fclose(fileID);
